...
struct ConditionMacroTests {
  @Test("#expect() macro",
    arguments: [
      ...
      ##"#expect(2 > 1)"##:
        ##"Testing.__checkBinaryOperation(2, { lhs, rhs in lhs > rhs() }, 1, sourceCode: .__fromBinaryOperation("2", ">", "1"), comments: [], isRequired: false, sourceLocation: Testing.SourceLocation()).__expected()"##,
      ##"#expect(((true || false) && true) || Bool.random())"##:
        ##"Testing.__checkBinaryOperation(((true || false) && true), { lhs, rhs in lhs || rhs() }, Bool.random(), sourceCode: .__fromBinaryOperation("((true || false) && true)", "||", "Bool.random()"), comments: [], isRequired: false, sourceLocation: Testing.SourceLocation()).__expected()"##,
      ##"#expect(9 > 8 && 7 > 6, "Some comment")"##:
        ##"Testing.__checkBinaryOperation(9 > 8, { lhs, rhs in lhs && rhs() }, 7 > 6, sourceCode: .__fromBinaryOperation("9 > 8", "&&", "7 > 6"), comments: ["Some comment"], isRequired: false, sourceLocation: Testing.SourceLocation()).__expected()"##,
      ##"#expect(!Bool.random())"##:
      ...
      ##"#expect((true && false))"##:
        ##"Testing.__checkBinaryOperation(true, { lhs, rhs in lhs && rhs() }, false, sourceCode: .__fromBinaryOperation("true", "&&", "false"), comments: [], isRequired: false, sourceLocation: Testing.SourceLocation()).__expected()"##,
      ##"#expect(try x())"##:
  ...
  @Test("#require() macro",
    arguments: [
      ...
      ##"#require(2 > 1)"##:
        ##"Testing.__checkBinaryOperation(2, { lhs, rhs in lhs > rhs() }, 1, sourceCode: .__fromBinaryOperation("2", ">", "1"), comments: [], isRequired: true, sourceLocation: Testing.SourceLocation()).__required()"##,
      ##"#require(((true || false) && true) || Bool.random())"##:
        ##"Testing.__checkBinaryOperation(((true || false) && true), { lhs, rhs in lhs || rhs() }, Bool.random(), sourceCode: .__fromBinaryOperation("((true || false) && true)", "||", "Bool.random()"), comments: [], isRequired: true, sourceLocation: Testing.SourceLocation()).__required()"##,
      ##"#require(9 > 8 && 7 > 6, "Some comment")"##:
        ##"Testing.__checkBinaryOperation(9 > 8, { lhs, rhs in lhs && rhs() }, 7 > 6, sourceCode: .__fromBinaryOperation("9 > 8", "&&", "7 > 6"), comments: ["Some comment"], isRequired: true, sourceLocation: Testing.SourceLocation()).__required()"##,
      ##"#require(!Bool.random())"##:
      ...
      ##"#require((true && false))"##:
        ##"Testing.__checkBinaryOperation(true, { lhs, rhs in lhs && rhs() }, false, sourceCode: .__fromBinaryOperation("true", "&&", "false"), comments: [], isRequired: true, sourceLocation: Testing.SourceLocation()).__required()"##,
      ##"#require(try x())"##:
  ...
  @Test("Unwrapping #require() macro",
    arguments: [
      ...
      ##"#require(nil ?? 123)"##:
        ##"Testing.__checkBinaryOperation(nil, { lhs, rhs in lhs ?? rhs() }, 123, sourceCode: .__fromBinaryOperation("nil", "??", "123"), comments: [], isRequired: true, sourceLocation: Testing.SourceLocation()).__required()"##,
      ##"#require(123 ?? nil)"##:
        ##"Testing.__checkBinaryOperation(123, { lhs, rhs in lhs ?? rhs() }, nil, sourceCode: .__fromBinaryOperation("123", "??", "nil"), comments: [], isRequired: true, sourceLocation: Testing.SourceLocation()).__required()"##,
  ...
  func macroExpansionInTestFunction() throws {
    ...
    let rawExpectedOutput = ##"""
      @Test("Random number generation") func rng() {
        let number = Int.random(in: 1 ..< .max)
        Testing.__checkBinaryOperation((number > 0 && foo() != bar(at: 9)), { lhs, rhs in lhs != rhs() }, !true, sourceCode: .__fromBinaryOperation("(number > 0 && foo() != bar(at: 9))", "!=", "!true"), comments: ["\(number) must be greater than 0"], isRequired: false, sourceLocation: Testing.SourceLocation()).__expected()
      }
    """##
